...
function mapDoc(row) {
  ...
  if (row.file_content) {
    doc.fileData = `data:${row.file_type || 'application/pdf'};base64,${row.file_content.toString('base64')}`;
  }
  return doc;
}
...
app.post('/api/trips/:invoice/documents', requireAuth, requireRegularUser, upload.single('file'), async (req, res) => {
  ...
  } else if (b.fileData && b.fileData.startsWith('data:')) {
...
app.put('/api/trips/:invoice/documents/:id', requireAuth, requireRegularUser, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    if (fileContent) {
      ...
      values = [docDate, b.description, b.billId, b.category, b.billAmount || 0, fileName, fileType, fileContent, docId, invoice];
    } else {
